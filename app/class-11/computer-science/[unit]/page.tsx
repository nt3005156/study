...
import data from '@/content/class-11-computer-science.json';
...
const UNIT_DEFAULT_LANG: Record<string, Lang> = {
  'unit-5': 'c',     // Programming Concepts and Logics (C language)
  'unit-6': 'html',  // Web Technology (HTML / CSS)
};
    ...
    <>
      <Breadcrumbs
        items={[
          ...
          { label: 'Computer Science', href: '/class-11/computer-science' },
          { label: heading },
        ]}
      />
      ...
      <article className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-24 pt-4" aria-label={heading}>
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-stone leading-relaxed max-w-4xl">
            {detailed
              ? detailed.note
              : `${unit.title}. Summary, key concepts and exercise answers from the Class 11 Computer Science textbook.`}
          </p>
        </header>
                ...
                {detailed ? (
                  <div className="rounded-xl bg-amber/5 border border-amber/25 p-4">
                    <p className="text-xs text-stone leading-relaxed">
                      ...
                      chapter ({detailed.exercise.short.length} short, {detailed.exercise.long.length} long
                      {detailProgrammingCount > 0 ? `, ${detailProgrammingCount} programming` : ''}
                      {detailPracticalCount > 0 ? `, ${detailPracticalCount} practical` : ''} and {detailed.exercise.mcq.length} multiple-choice questions with answer key) is given in full in
        ...
        <nav aria-label="Unit navigation" className="flex items-center justify-between gap-4 pt-10 border-t border-slate-200">
          {prev ? (
            <Link href={`/class-11/computer-science/${prev.unit_id}`} className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-slate-200 text-ink font-semibold shadow-sm hover:border-amber transition" aria-label={`Previous: ${prev.title}`}>
              <ArrowLeft size={18} /> {prev.chapter}
            </Link>
          ) : <span />}
          <Link href="/class-11/computer-science" className="text-sm font-semibold text-stone hover:text-ink">All chapters</Link>
          {next ? (
            <Link href={`/class-11/computer-science/${next.unit_id}`} className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-slate-200 text-ink font-semibold shadow-sm hover:border-amber transition" aria-label={`Next: ${next.title}`}>
