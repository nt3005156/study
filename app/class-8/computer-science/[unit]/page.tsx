...
import data from '@/content/class-8-computer-science.json';

type QA = { q: string; a: string };
type Concept = { type: string; term: string; text: string };
type Detailed = {
  heading: string;
  book: string;
  book_pages: string;
...
export default function UnitPage({ params }: { params: { unit: string } }) {
  ...
  // `detailed` is read through an explicit cast: the JSON's own inferred type
  // collapses empty arrays to never[], which would poison Detailed's fields.
  const detailed = (unit as unknown as { detailed?: Detailed }).detailed;
    ...
    <>
      <Breadcrumbs
        items={[
          ...
          { label: 'Computer Science', href: '/class-8/computer-science' },
          { label: heading },
        ]}
      />
      ...
      <article className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-24 pt-4" aria-label={heading}>
        ...
        <header className="mb-8">
          ...
          <p className="text-lg text-stone leading-relaxed max-w-4xl">
            {detailed
              ? detailed.note
              : `${unit.title}. Summary, key concepts and exercise answers from the Class 8 Computer Science textbook.`}
          </p>
        </header>
        ...
        {!detailed ? (
          <>
            ...
            {(unit.concepts as Concept[]).length > 0 ? (
            <section className="mb-10">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {(unit.concepts as Concept[]).map((c, i) => (
        ...
        <nav aria-label="Unit navigation" className="flex items-center justify-between gap-4 pt-10 border-t border-slate-200">
          {prev ? (
            <Link href={`/class-8/computer-science/${prev.unit_id}`} className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-slate-200 text-ink font-semibold shadow-sm hover:border-amber transition" aria-label={`Previous: ${prev.title}`}>
              <ArrowLeft size={18} /> {prev.chapter}
            </Link>
          ) : <span />}
          <Link href="/class-8/computer-science" className="text-sm font-semibold text-stone hover:text-ink">All chapters</Link>
          {next ? (
            <Link href={`/class-8/computer-science/${next.unit_id}`} className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-white border border-slate-200 text-ink font-semibold shadow-sm hover:border-amber transition" aria-label={`Next: ${next.title}`}>
