...
export function Breadcrumbs({ items, center = false }: { items: { label: string; href?: string }[]; center?: boolean }) {
  return (
    <nav aria-label="Breadcrumb" className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-3 no-print">
      <ol className={`flex items-center gap-1.5 text-xs sm:text-sm text-stone flex-wrap ${center ? 'justify-center' : ''}`}>
        <li><Link href="/" className="hover:text-ink transition flex items-center gap-1">{center ? null : <Home size={14} />} Home</Link></li>
        {items.map((item, i) => (
